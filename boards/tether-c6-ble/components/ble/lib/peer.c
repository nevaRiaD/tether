...
int
ble_device_type_from_name(const char *name)
{
    if (name != NULL && strncmp(name, "Tether Tag", 10) == 0) {
        return DEVICE_TYPE_ACCEL;
    }
    return DEVICE_TYPE_RSSI;
}

int
peer_add(uint16_t conn_handle, uint8_t device_type)
{
    struct peer *peer;
